...
import model.NotificationProject.EmailConnection;
...
public class NotificationSystem implements NotificationService {
	...
	@GET
	@Path("/{subject}/{language}/getOne")
	public NotificationTemplate getNotification(@PathParam("subject") String subject, @PathParam("language") String language) {
		NotificationTemplate notification = new NotificationTemplate();
		NotificationConnection connection = new NotificationConnection();
		notification = connection.getNotification(subject, language);
		return notification;
	}
